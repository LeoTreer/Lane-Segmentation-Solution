...
# input :
# image, label=None ,new_size, offset
def crop_resize_data(image, label=None, image_size=(1024, 384), offset=690):
    ...
    if label is not None:
        ...
        train_image = cv2.resize(
            roi_image, image_size, interpolation=cv2.INTER_LINEAR)  # image做的是线性插值
        train_label = cv2.resize(
            roi_label, image_size, interpolation=cv2.INTER_NEAREST)  # label做的是最邻近插值
        return train_image, train_label
    else:
        train_image = cv2.resize(
            roi_image, image_size, interpolation=cv2.INTER_LINEAR)
        return train_image
        ...
        if np.random.uniform(0, 1) > 0.5:  # np.random.uniform(0,1)在0-1之间随机取值
            ...
            seq = iaa.Sequential([iaa.OneOf([
                iaa.AdditiveGaussianNoise(
                    scale=(0, 0.2 * 255)),  # 加一个高斯噪音 noises
...
class CutOut(object):
    ...
    def __call__(self, sample):
        ...
        xmin, ymin, xmax, ymax = max(0, xmin), max(
            0, ymin), min(w, xmax), min(h, ymax)
...
def expand_resize_data(prediction=None, submission_size=(3384, 1710), offset=690):
    ...
    expand_mask = cv2.resize(
        pred_mask, (submission_size[0], submission_size[1] - offset), interpolation=cv2.INTER_NEAREST)
...
def expand_resize_color_data(prediction=None, submission_size=(3384, 1710), offset=690):
    ...
    color_expand_mask = cv2.resize(
        color_pred_mask, (submission_size[0], submission_size[1] - offset), interpolation=cv2.INTER_NEAREST)
    color_submission_mask = np.zeros(
        (submission_size[1], submission_size[0], 3), dtype='uint8')
